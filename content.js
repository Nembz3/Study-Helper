(()=>{
  ...
  function candidateText(){
    ...
    for(const anchor of anchors){
      ...
      for(let i=0;i<7&&el;i++,el=el.parentElement){
        ...
        const r=el.getBoundingClientRect();
        if(text.length>=15&&text.length<=1800&&r.height>=80&&r.height<=innerHeight*1.25){
          candidates.push(el);
        }
      }
    }

    // Prefer the smallest useful container closest to the middle of the current viewport.
    ...
    for(const el of candidates){
      const text=clean(el.innerText);
      // Ignore containers that mostly look like navigation/page chrome.
      if(text.length>20) return text.slice(0,1800);
    }

    // Fallback: only use text actually inside the viewport.
    const fallback=all
      .filter(el=>{
  ...
  function mount(question){
    ...
    const sig=question.slice(0,900);
    ...
    root.id=ROOT_ID;
    root.innerHTML=`<div class="sh-card"><div class="sh-head"><strong>Study Helper V3.1</strong><button class="sh-close">×</button></div><div class="sh-label">Current question detected</div><div class="sh-question"></div><div class="sh-actions"><button class="sh-hint">Hint</button><button class="sh-explain">Explain</button><button class="sh-answer">Analyse answer</button></div><div class="sh-status"></div><div class="sh-response"></div></div>`;
  ...
  addEventListener("scroll",()=>{clearTimeout(debounce);debounce=setTimeout(scan,250)},{passive:true});
